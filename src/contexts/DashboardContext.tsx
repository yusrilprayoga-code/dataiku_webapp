...
export const DashboardProvider = ({ children }: { children: ReactNode }) => {
	...
	const [plotFigure, setPlotFigure] = useState<PlotFigure>({
		data: [],
		layout: {},
	});
	const [plotHeaderFigure, setPlotHeaderFigure] = useState<PlotFigure>({
		data: [],
		layout: {},
	});
	...
	const fetchWellColumns = useCallback(
		async (wells: string[]) => {
			if (!apiUrl) {
				console.warn("API URL not set, cannot fetch well columns");
				return;
			}
			const wellsToFetch = wells.filter(
				(w) =>
			...
			if (isDataPrep) {
				...
				if (!wellsDir) {
					console.warn(
						"wellsDir is not set; cannot fetch columns for dashboard."
					);
					return;
				}
			...
			try {
				...
				if (
					...
					throw new Error(
						"API returned empty or invalid data for well columns."
					);
				}
	...
	const fetchPlotData = useCallback(async () => {
		...
		setPlotHeaderFigure({ data: [], layout: {} });

		const isDataPrepFlow = !!selectedFilePath;
		if (!isDataPrepFlow && selectedWells.length === 0) {
			setPlotFigure({
				data: [],
				layout: { title: { text: "Please select a well from the list." } },
			});
		...
		if (isDataPrepFlow && !selectedFilePath) {
			setPlotFigure({
				data: [],
				layout: { title: { text: "Please select a file from the browser." } },
			});
			setIsLoadingPlot(false);
			return;
		}
		...
		if (isDataPrepFlow) {
			switch (plotType) {
				case "normalization-prep":
					...
					break;
				case "depth-matching":
					endpointPath = "/api/get-depth-matching-prep-plot";
					break;
				case "smoothing-prep":
					endpointPath = "/api/get-smoothing-prep-plot";
					break;
				case "fill-missing-prep":
					endpointPath = "/api/get-fill-missing-plot";
					break;
				case "splicing":
					endpointPath = "/api/get-splicing-plot";
					break;
				case "trimming":
					endpointPath = "/api/get-trimming-plot";
					break;
				case "custom":
					endpointPath = "/api/get-custom-plot";
					break;
				default:
			...
			if (plotType === "custom") {
				console.log(
					`selectedCustomColumns: ${selectedCustomColumns} dan selectedFilePath: ${selectedFilePath}`
				);
				requestBody = {
					custom_columns: selectedCustomColumns,
		...
		try {
			...
			if (!response.ok) {
				...
				throw new Error(errorData.error || "Failed to fetch plot from server");
			}

			const plotData = await response.json();

			// Validasi dan set state untuk kedua plot
			if (plotData && plotData.main_plot && plotData.header_plot) {
				setPlotFigure({
					data: plotData.main_plot.data || [],
					layout: plotData.main_plot.layout || {},
				});
				setPlotHeaderFigure({
					data: plotData.header_plot.data || [],
					layout: plotData.header_plot.layout || {},
				});
				console.log(plotData.header_plot);
			} else {
				// Jika backend hanya mengembalikan satu plot (untuk endpoint lama)
				const parsed =
				...
				if (parsed && (parsed.data || parsed.layout)) {
					...
					setPlotHeaderFigure({ data: [], layout: {} }); // Kosongkan header
				} else {
