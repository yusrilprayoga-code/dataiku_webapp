...
const createInitialParameters = (intervals: string[]): ParameterRow[] => {
	...
	const allPossibleParams: Omit<ParameterRow, "values">[] = [
		// Parameter input oleh pengguna (angka)
		{
			...
			location: "Constant",
			mode: "Input",
			comment: "Coeff for saturation equation: A",
			unit: "",
			name: "A",
			isEnabled: true,
		},
		{
			...
			location: "Constant",
			mode: "Input",
			comment: "Coeff for saturation equation: M",
			unit: "",
			name: "M",
			isEnabled: true,
		},
		{
			...
			location: "Constant",
			mode: "Input",
			comment: "Coeff for saturation equation: N",
			unit: "",
			name: "N",
			isEnabled: true,
		},
		...
		// Parameter input log (dropdown)
		{
			...
			location: "Log",
			mode: "Input",
			comment: "Apparent Water Resistivity",
			unit: "OHMM",
			name: "RWA",
			isEnabled: true,
		},
		...
		},
		{
			...
			location: "Log",
			mode: "Input",
			comment: "Unflushed Zone Resistivity",
			unit: "OHMM",
			name: "RT",
			isEnabled: true,
		},
		...
		// Parameter output (tidak bisa diubah pengguna)
		{
			...
			location: "Log",
			mode: "Output",
			comment: "Grouping ID based on IQUAL",
			unit: "",
			name: "GROUP_ID",
			isEnabled: true,
		},
		...
		},
		{
			...
			location: "Log",
			mode: "Output",
			comment: "PHIE vs RT-R0 Gradient",
			unit: "",
			name: "PHIE_RTR0_GRAD",
			isEnabled: true,
		},
...
export default function RtRoParams() {
	...
	const combinedColumns = useMemo(() => {
		if (
			...
			return [];
		// PERBAIKAN: Menggunakan format key `${well}.csv` agar konsisten
	...
	// BARU: Buat daftar opsi yang sudah di-filter sebelumnya dengan useMemo
	const rwaOptions = useMemo(
		() => combinedColumns.filter((col) => col.toUpperCase().includes("RWA")),
		[combinedColumns]
	);
