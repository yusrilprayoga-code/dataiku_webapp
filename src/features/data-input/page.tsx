...
import { FileTextIcon, Folder as FolderIcon, Inbox, CheckCircle, Loader2 } from 'lucide-react';
import DataTablePreview from '@/features/data-input/components/DataInputView';

export default function DataInputUtamaPage() {
  const router = useRouter();
  const [isLoading, setIsLoading] = useState(true); // Mulai dengan state loading

  // Ambil semua state dan action dari store
  const { stagedStructure, setStagedStructure, qcResults, setQcResults, handledFiles, addHandledFile, clearAllData } = useAppDataStore();
  ...
  // --- PERBAIKAN PENTING: LOGIKA UNTUK MEMUAT DATA ---
  useEffect(() => {
    // Cek jika data sudah ada di store (misalnya dari navigasi lokal yang cepat)
    if (stagedStructure) {
      setIsLoading(false);
      return;
    }

    // Jika tidak, coba ambil dari sessionStorage
    const storedDataString = sessionStorage.getItem('stagedStructureForProcessing');

    if (storedDataString) {
      try {
        const parsedData: StagedStructure = JSON.parse(storedDataString);
        setStagedStructure(parsedData); // Masukkan data ke Zustand
        sessionStorage.removeItem('stagedStructureForProcessing'); // Hapus agar tidak dipakai lagi
      } catch (error) {
        console.error("Gagal mem-parsing data dari sessionStorage", error);
        router.replace('/'); // Jika data rusak, kembali ke awal
      } finally {
        setIsLoading(false); // Selesai loading
      }
    } else {
      // Jika tidak ada data sama sekali, redirect
      console.warn("Tidak ada data staged di store atau storage. Mengarahkan ke halaman utama.");
      router.replace('/');
    }
  }, [stagedStructure, router, setStagedStructure]);
