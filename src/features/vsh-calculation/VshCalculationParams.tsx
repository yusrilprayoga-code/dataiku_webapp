			...
			try {
				if (fetchPerZone && isUsingZones) {
					...
					const zonePromises = selectedZones.map(async (zone) => {
						...
						if (!response.ok) {
							console.error(
								`Gagal mengambil nilai default GR untuk zone ${zone}.`
							);
							return null;
						}
						...
						prevParams.map((param) => {
							if (param.name === "GR_MA") {
								const newValues = Object.fromEntries(
									Object.keys(param.values).map((key) => [key, data.gr_ma])
								);
								return { ...param, values: newValues };
							}
							if (param.name === "GR_SH") {
								const newValues = Object.fromEntries(
									Object.keys(param.values).map((key) => [key, data.gr_sh])
								);
								return { ...param, values: newValues };
							}
							return param;
				...
				<div className="flex-shrink-0 mb-6 p-4 border rounded-lg bg-gray-50">
					...
					{isUsingZones && (
						<div className="flex items-center gap-2 mt-4 text-sm">
							...
							<label htmlFor="fetch-per-zone">
								Fetch param values for each zone individually
							</label>
							<input
								id="fetch-per-zone"
								type="checkbox"
							...
							<tbody className="bg-white">
								{parameters.map((param) => (
									<tr
										...
										<td
											className={`px-3 py-2 border-r ${getRowBgColor(
												param.location,
												param.mode
											)}`}>
											{param.location}
										...
										<td
											className={`px-3 py-2 border-r font-semibold ${getRowBgColor(
												param.location,
												param.mode
											)}`}>
											{param.name}
											...
											selectedZones.map((zone) => (
												<td
													...
													<input
														...
														onChange={(e) =>
															handleValueChange(param.id, zone, e.target.value)
														}
														className="w-full min-w-[150px] p-1 bg-white"
													/>
												</td>
