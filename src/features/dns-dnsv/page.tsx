...
export default function DnsDnsvParams() {
	...
	const depthOptions = useMemo(
		() => combinedColumns.filter((col) => col.toUpperCase().includes("DEPTH")),
		[combinedColumns]
	);
	const nphiOptions = useMemo(
		() => combinedColumns.filter((col) => col.toUpperCase().includes("NPHI")),
		[combinedColumns]
	);
	const rhobOptions = useMemo(
												...
												<td
													...
													{useDropdown ? (
														...
														<input
															type="text"
															value={String(currentValue)}
															className="w-full min-w-[100px] p-1 bg-white" // Styled for output
														/>
													)}
												</td>
											);
