...
const createInitialParameters = (intervals: string[]): ParameterRow[] => {
    ...
    const allPossibleParams: Omit<ParameterRow, 'values'>[] = [
        { id: 1, location: 'Interval', mode: 'Input', comment: 'Shale resistivity', unit: 'OHMM', name: 'RTSH', isEnabled: true },
        { id: 2, location: 'Interval', mode: 'Input', comment: 'Coeff for saturation equation: A', unit: '', name: 'A', isEnabled: true },
        { id: 3, location: 'Interval', mode: 'Input', comment: 'Coeff for saturation equation: M', unit: '', name: 'M', isEnabled: true },
        { id: 4, location: 'Interval', mode: 'Input', comment: 'Coeff for saturation equation: N', unit: '', name: 'N', isEnabled: true },
        ...
        { id: 8, location: 'Log', mode: 'Input', comment: 'Formation temperature', unit: 'DEGC', name: 'FTEMP', isEnabled: true },
...
export default function SwgradParams() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            alert(result.message || "Proses SWGRAD berhasil!");
                                ...
                                {parameters.map((param) => (
                                    <tr key={param.id} className={`border-b border-gray-200 ${getRowBgColor(param.location, param.mode)}`}>
                                        ...
                                        {selectedIntervals.map(interval => {
                                            ...
                                            if (param.name === 'PHIE') filteredOptions = combinedColumns.filter(c => c.includes('PHIE'));
                                            if (param.name === 'VSH') filteredOptions = combinedColumns.filter(c => c.includes('VSH'));
