...
const createInitialParameters = (): ParameterRow[] => {
	...
	const smoothingParams: Omit<ParameterRow, "values">[] = [
		...
		},
		{
			...
			location: "Parameter",
			mode: "Input",
			comment: "Size of smooth window (odd number)",
			unit: "",
			name: "WINDOW",
			isEnabled: true,
		},
	...
	useEffect(() => {
		if (currentLogIn) {
			setParameters((prev) =>
				prev.map((p) =>
					p.name === "LOG_OUT"
						? { ...p, values: { default: `${currentLogIn}_SM` } }
						: p
				)
			);
		}
			...
			<form
				...
				<div className="flex-shrink-0 mb-6 p-4 border rounded-lg bg-gray-50">
					<div className="flex justify-end gap-2">
						...
						<button
							type="submit"
							className="px-6 py-2 rounded-md text-white font-semibold bg-blue-600 hover:bg-blue-700 flex items-center justify-center min-w-[80px]"
							disabled={isSubmitting || selectedWells.length === 0}>
